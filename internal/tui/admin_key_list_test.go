...
// skeletonListVault wraps a memory store but List returns only
// Provider+Account (no payloads), mirroring the production keychain
// backend's lightweight-skeleton optimization. This is the exact
// divergence that hid the prod-only bug where admin-key creds were
// invisible in the entity list because the filter checked AdminKey
// payload fields nil-on-skeletons.
type skeletonListVault struct {
	inner *memory.Store
}

func (s *skeletonListVault) Get(provider, account string) (*vault.Credential, error) {
	return s.inner.Get(provider, account)
}
func (s *skeletonListVault) Set(c *vault.Credential) error { return s.inner.Set(c) }
func (s *skeletonListVault) Delete(provider, account string) error {
	return s.inner.Delete(provider, account)
}
func (s *skeletonListVault) List() ([]*vault.Credential, error) {
	full, err := s.inner.List()
	if err != nil {
		return nil, err
	}
	skel := make([]*vault.Credential, 0, len(full))
	for _, c := range full {
		// Production keychain List drops every field except Provider
		// and Account. Mirror that exactly so this test fails if the
		// entity-list ever regresses to relying on List-side payload.
		skel = append(skel, &vault.Credential{
			Provider: c.Provider,
			Account:  c.Account,
		})
	}
...
// Regression for the prod-only bug: entity list must use Get on each
// candidate (not just rely on List) because List returns skeletons
// in the keychain backend.
func TestAdminKeyList_FindsCredsViaGet_NotJustList(t *testing.T) {
	mem := memory.New()
	_ = mem.Set(&vault.Credential{
		Type: vault.TypeAdminKey, Provider: "openai", Account: "image-gen",
		AdminKey: &vault.AdminKeyData{
			OrgID: "org-test", ProjectName: "another", ProjectID: "proj_X",
			KeyID: "svc_X", KeyMaterial: "sk-test-secret",
		},
	})
	v := &skeletonListVault{inner: mem}
	...
	// Should have exactly one project row: image-gen.
	var keyRows []adminKeyListRow
	for _, r := range m.rows {
	...
	if len(keyRows) != 1 {
		t.Fatalf("expected 1 key row from skeleton-list vault, got %d (rows: %+v)", len(keyRows), m.rows)
	}
	if keyRows[0].account != "image-gen" {
		t.Errorf("account = %q, want image-gen", keyRows[0].account)
	}
	if keyRows[0].projectName != "another" {
		t.Errorf("projectName not loaded via Get: %+v", keyRows[0])
	}
	if keyRows[0].keyHint == "" {
		t.Errorf("keyHint empty — Get didn't load AdminKey payload: %+v", keyRows[0])
	}
}
