...
```
- Routing: exact host match first, then suffix match (e.g. `*.googleapis.com` → `{google, bearer}`). The Google suffix rule transparently covers Vertex AI (`{region}-aiplatform.googleapis.com`) and the API Keys mint endpoint (`apikeys.googleapis.com`) — OAuth bearer with the `cloud-platform` scope authenticates both. AI Studio (`generativelanguage.googleapis.com`) is an *exact-match override* mapped to `{google-aistudio, url_param_key}`: charon attaches `?key=<cred.AIStudio.KeyMaterial>` instead of an Authorization header. The credential lookup uses `VaultProvider="google"` so the same Google OAuth credential carries both bearer tokens (for Vertex/Workspace) and the AI Studio key sidecar.
